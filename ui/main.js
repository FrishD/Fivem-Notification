...
function appendNotification(containerId, type, message){
  ...
  if(message.length >= 65){
    $.post(`https://S&S_JobNotifications/limitexceded`, JSON.stringify({
      id: idplayer
    })); return;
  }
...
function getClassInfo(type){
  ...
  switch(type){
    ...
    case "police": info.className = "fa-handcuffs";
      break;
    case "ambulance": info.className = "fa-truck-medical";
      break;
    case "mechanic": info.className = "fa-wrench";
      break;
